...
export function buildAgentSystemPrompt(input: BuildAgentPromptInput) {
  return [
    ...
    '- 当你得到稳定、长期有价值的结论时，按范围选择写入工具：',
    '  · 节点专属知识 → memory.write_node_memory',
    '  · 分组共享知识 → memory.write_group_memory',
    '  · 跨节点/跨分组的全局知识 → memory.write_global_memory',
    '- 若只需更新记忆文档中某个小节（如"关键事实"），优先使用 memory.update_memory_section，避免覆盖整个文档。',
    ...
    '- 当你需要用户补充参数、做选择、确认方案时，必须调用 interaction.request 工具。',
    '- 不要直接用 assistant 文本向用户提问。',
